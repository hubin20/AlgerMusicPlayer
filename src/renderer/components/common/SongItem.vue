  ...
  >
    ...
    <div class="song-item-content" :class="{ 'song-item-content-compact': compact }">
      <div v-if="list" class="song-item-content-wrapper">
        ...
        <span v-if="item.source === 'other'" class="source-tag other-tag ml-2">其他</span>
        <span v-else-if="item.source === 'netease'" class="source-tag netease-tag ml-2">网易</span>
      ...
      <template v-else>
        <div class="song-item-content-title" @dblclick="playMusicEvent(item)">
          <n-ellipsis class="text-ellipsis" line-clamp="1" :class="{ 'text-green-500': isPlaying }">
            {{ item.name }}
            <span v-if="item.source === 'other'" class="source-tag other-tag ml-1">其他</span>
            <span v-else-if="item.source === 'netease'" class="source-tag netease-tag ml-1">网易</span>
          </n-ellipsis>
        </div>
...
.netease-tag {
  background-color: #c20c0c; // 网易红
}

.other-tag {
  background-color: #2d8cf0; // 之前酷我的蓝色，或选择一个通用颜色
}

/* 如果之前有 kw-tag 或 bilibili-tag 的特定样式，可以移除或调整 */
.kw-tag {
  // background-color: #2d8cf0; // 示例：酷我蓝
}

.bilibili-tag {
  // background-color: #fb7299; // 示例：B站粉
}
</style>
