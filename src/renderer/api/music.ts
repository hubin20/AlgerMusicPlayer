...
export const getParsingMusicUrl = async (id: number, data: SongResult) => {
  ...
  // 对于 Web 环境 (Vercel部署)，使用 /match 路径，并可能需要传递音源参数
  // 根据 hubin20/UNM-Server 的 README，它支持 server 参数，例如 &server=kuwo,kugou
  // 我们暂时只传id，如果需要指定音源，这里的逻辑需要进一步扩充
  return requestMusic.get<any>('/match', { params: { id } });
};
